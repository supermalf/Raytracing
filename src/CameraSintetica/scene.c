...
struct _Scene
{
   ...
   /**
     * Um marcador de posicao 3D na cena utilizado para testar alg de visao computacional
     */
   Vector marker;
};
...
Scene* sceLoad( const char *filename )
{
	...
	while( fgets( buffer, sizeof(buffer), file ) ) 
	{
	  	...
	  	else if( sscanf( buffer, "BOX %d %lf %lf %lf %lf %lf %lf\n", &material, &pos1.x, &pos1.y, &pos1.z, &pos2.x, &pos2.y, &pos2.z ) == 7 ) 
		{
	  	...
	  	else if( sscanf( buffer, "MARKER %lf %lf %lf \n", &pos1.x, &pos1.y, &pos1.z ) == 3 ) 
		{
         scene->marker = pos1;
		} 
		else
...
Vector sceGetMarker( Scene* scene )
{
   return scene->marker;
}
